    ...
    <div className={`w-full px-5  bg-[#f4f2f1] `}> {/* 🔥 Ye color same rahega */}
      <div
        className={`fixed top-0 left-1/2 transform -translate-x-1/2 ${
          scrolling ? "max-w-5xl justify-center flex items-center  lg:translate-y-4 shadow-md   md:bg-white  lg:bg-white md:bg-white  " : "max-w-6xl   py-7  md:bg-[#f4f2f1]  lg:bg-[#f4f2f1] sm:bg-white  bg-gray-700"
        } w-full flex justify-between items-center  transition-all duration-300 z-50  lg:rounded-lg md:rounded-none lg:py-3 md:py-7   px-6 bg-[#f4f2f1] py-1  `} 
      >
        <motion.div
    ...
    >
      ...
      />
        ...
        <motion.ul
            ...
            animate={{ opacity: [0, 1], x: scrolling ? [8, 0] : [0, 8] }} // 🔄 Smooth transition
          ...
          >
            {navItems.map((item, index) => (
              <motion.li
                ...
                animate={{opacity: [0, 1], y: scrolling ? [0, 0] : [0, 3] }} // 🌀 Smooth move
                transition={{ duration: 0.7 }}
              >
                {item}
              </motion.li>
